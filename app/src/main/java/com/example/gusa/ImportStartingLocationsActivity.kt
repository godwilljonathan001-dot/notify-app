...
import com.example.gusa.databinding.ActivityImportstartinglocationsBinding
...
class ImportStartingLocationsActivity : FragmentActivity(), OnMapReadyCallback {
    private lateinit var binding: ActivityImportstartinglocationsBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityImportstartinglocationsBinding.inflate(layoutInflater)
    ...
    private fun switchMode(mode: ImportMode) {
        ...
        if (mode == ImportMode.RAW) {
            ...
            binding.tvAiSubTitle.text = "Paste structured hub data (ID, Name, Lat, Lng)"
            binding.etRawData.hint = "Paste starting location records here..."
            binding.btnValidate.text = "Validate Hubs"
            ...
            binding.etRawData.hint = "Paste unstructured depot descriptions..."
            binding.btnValidate.text = "Analyze Hubs with AI"
        }
    }
    ...
    private fun loadDropdownData() {
        lifecycleScope.launch {
            try {
                ...
                binding.spinnerManualUni.setAdapter(ArrayAdapter(this@ImportStartingLocationsActivity, android.R.layout.simple_dropdown_item_1line, unis))
            } catch (e: Exception) {
                Toast.makeText(this@ImportStartingLocationsActivity, "Sync error", Toast.LENGTH_SHORT).show()
            }
        }
    }
    ...
    private fun setupListeners() {
        ...
        binding.btnPickOnMap.setOnClickListener {
            ...
            binding.btnPickOnMap.text = if (binding.mapContainer.visibility == View.VISIBLE) "Collapse Map" else "Mark on Infrastructure Map"
        }
    ...
    private fun processManualEntry() {
        val hid = binding.etManualId.text.toString().trim()
        val name = binding.etManualName.text.toString().trim()
        val lat = binding.etManualLat.text.toString().toDoubleOrNull()
        val lng = binding.etManualLng.text.toString().toDoubleOrNull()
        
        if (hid.isEmpty() || name.isEmpty() || lat == null || lng == null) {
            showSnackbar("ID, Name, and Coordinates are mandatory")
            return
        }

        val obj = JSONObject().apply {
            put("startingLocationId", hid)
            put("name", name)
            put("latitude", lat)
            put("longitude", lng)
    ...
    private fun startValidationWorkflow(rawText: String) {
        lifecycleScope.launch {
            ...
            setValidationState(isProcessing = true)
            try {
                updateProgress("AI Architect identifying hub coordinates...")
                val responseStr = GeminiService.parseImportData(rawText, "starting_locations")
    ...
    private suspend fun validateParsedData(data: JSONArray) {
        ...
        for (i in 0 until data.length()) {
            ...
            val hid = obj.optString("startingLocationId")
            ...
            if (hid.isEmpty() || lat.isNaN() || lng.isNaN()) {
                status = "Error"
                issues.add("Incomplete Record")
                errCount++
            } else {
                val existing = db.collection("starting_locations").document(hid).get().await()
                if (existing.exists()) {
                    status = "Duplicate"
                    issues.add("ID taken")
                    dupCount++
    ...
    private fun updateStatsUi(total: Int, dups: Int, errs: Int) {
        ...
        binding.tvStatTotal.text = "Hubs: $total"
    ...
    private fun generatePreviewTable() {
        ...
        arrayOf("Status", "Hub ID", "Name", "Latitude", "Longitude").forEach { 
        ...
        validatedData.take(50).forEach { obj ->
            ...
            row.addView(TextView(this).apply { text = obj.optString("startingLocationId"); setPadding(16, 12, 16, 12) })
    ...
    private fun executeBatchUpload() {
        ...
        lifecycleScope.launch {
            try {
                ...
                val importId = "HUB-BATCH-${System.currentTimeMillis()}"
                currentImportId = importId
                importedIds.clear()

                toUpload.forEach { hub ->
                    val id = hub.optString("startingLocationId")
                    val lat = hub.optDouble("latitude")
                    val lng = hub.optDouble("longitude")
                    val map = hashMapOf(
                        "startingLocationId" to id,
                        "name" to hub.optString("name"),
                        "latitude" to lat,
                        "longitude" to lng,
                        "geoPoint" to GeoPoint(lat, lng),
                        "universityId" to hub.optString("universityId"),
                        "importId" to importId,
                        "createdAt" to Timestamp.now()
                    )
                    db.collection("starting_locations").document(id).set(map).await()
                    importedIds.add(id)
                }
                updateStepIndicator(4)
                showCompletion(importedIds.size)
            } catch (e: Exception) {
                showSnackbar("Hub deployment failed")
    ...
    private fun showCompletion(count: Int) {
        ...
        binding.tvImportSummary.text = "Deployed $count hubs.\nBatch ID: $currentImportId"
    }

    private fun undoLastImport() {
        lifecycleScope.launch {
            try {
                importedIds.forEach { db.collection("starting_locations").document(it).delete().await() }
                Toast.makeText(this@ImportStartingLocationsActivity, "Reverted", Toast.LENGTH_SHORT).show()
                finish()
